...
export const getPublicDestinations = createServerFn().handler<Promise<PublicDestination[] | undefined>>(async () => {
  const data = await createDestinationRepository(db).getPublicDestinationsList();

  console.log("Fetched destinations:", data, "from server function");
